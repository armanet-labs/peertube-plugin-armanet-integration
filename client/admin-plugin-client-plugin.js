...
async function handleChannelsList (registerHook, peertubeHelpers, registerSettingsScript) {
  registerHook({
    ...
    handler: async ({ ...rest }) => {
      ...
      try {
        const chs = await fetch('/api/v1/video-channels', {method: 'GET'})
        ...
        const syncChannels = async (channelsListEndpoint, channelsData) => {
          ...
          if (syncResponse) {
            peertubeHelpers.notifier.success(`${syncResponse.syncCount} channels successfully synchronized`, 'Channels Sync', 7000)

            if(syncResponse.errorData.length > 0) {
              peertubeHelpers.notifier.error(`Failed to sync ${syncResponse.errorData.length} channels`, 'Channels Sync', 7000)
            }
          } else {
            peertubeHelpers.notifier.error('could not sync channels', 'Channels Sync', 7000)
