...
export const setupResourceHints = () => {
  ...
  const existingHints = new Set(
    Array.from(document.querySelectorAll('link[rel="dns-prefetch"], link[rel="preconnect"]'))
...
export const handleCompanions = async (settings, clientDebug) => {
  const enabledCompanions = Object.entries(settings.companion)
    .filter(([_, config]) => config.enabled && config.adUnit);
